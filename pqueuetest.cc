...
void BasicPQueueTest()
{
    ...
    std::cout << std::boolalpha;	// configure stream to print booleans as true/false instead of 1/0
    std::cout << std::endl << "-----------   Testing Basic PQueue functions -----------" << std::endl;
    std::cout << "The pqueue was just created.  Is it empty? " << pq.isEmpty() << std::endl;

    std::cout << std::endl << "Now enqueuing integers from 1 to 10 (increasing order)" << std::endl;
    for (int i = 1; i <= 10; i++)
        pq.enqueue(i);

    std::cout << "Pqueue should not be empty.  Is it empty? " << pq.isEmpty() << std::endl;
    std::cout << "Pqueue should have size = 10.  What is size? " << pq.size() << std::endl;
    pq.printDebuggingInfo();

    std::cout << "Dequeuing the top 5 elements: ";
    for (int j = 0; j < 5; j++)
        std::cout << pq.dequeueMax() << " ";
    std::cout << std::endl << "Pqueue should have size = 5.  What is size? " << pq.size() << std::endl;
    pq.printDebuggingInfo();

    std::cout << std::endl << "Dequeuing all the rest: ";
    while (!pq.isEmpty())
        std::cout << pq.dequeueMax() << " ";
    std::cout << std::endl << "Pqueue should be empty.  Is it empty? " << pq.isEmpty() << std::endl;
    pq.printDebuggingInfo();
...
void MorePQueueTest()
{
    ...
    std::cout << std::endl << "-----------   More pqueue testing functions -----------" << std::endl;
...
void PQueueSortTest()
{
    ...
    std::cout << std::endl << "-----------   Testing use of pqueue to sort  -----------" << std::endl;

    std::cout << "Enqueuing " << SortSize << " numbers into pqueue in increasing order." << std::endl;
    for (int i = 0; i < SortSize; i++) array[i] = i;
    PQSort(array, SortSize);
    std::cout << "Using dequeue to pull out numbers in sorted order.  Are they sorted? " <<
        ArrayIsSorted(array, SortSize) << std::endl;

    std::cout << "Enqueuing " << SortSize << " random values into the pqueue." << std::endl;
